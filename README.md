...
| `refund_order` | Full refund on a processed order. ✅ Creating a refund (`ReturnsRefunds/CreateRefund`) and the eligibility gate are live-proven 18 Sep 2026. ⚠️ Pushing the refund to a sales channel (`ActionRefund`, `push_to_channel=True`) has never been shown to actually reach a channel — every live push carries a `channel_push_warning`. Live runs re-read the order's refund headers and report `outcome`: confirmed/unconfirmed/read_back_failed/rate_limited |
| `refund_order_lines` | Partial refund of specific line items. ✅ `CreateRefund` and the eligibility gate are live-proven 18 Sep 2026. ⚠️ Pushing to a sales channel (`ActionRefund`, `push_to_channel=True`) has never been shown to actually reach a channel — same `channel_push_warning` and read-back `outcome` as `refund_order`. Refuses a partial `quantity` without an explicit `amount`, which would otherwise silently refund the whole line's cost |

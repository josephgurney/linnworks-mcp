...
**Current version: 1.25.0** — 73 tools. See `pyproject.toml` for full metadata.
...
| `refresh_channel_listing(skus, sub_source="SWH Shopify", action=None, confirmed_count, dry_run=True)` | `GenericListings/GetConfiguratorsInfoPaged` + `Inventory/GetInventoryItemChannelSKUs` + `GenericListings/OpenTemplatesByInventory` (read) → `GenericListings/ProcessTemplates` (write) | 25 | **Revise counterpart to `list_to_shopify` (issue #20)** — re-pushes EXISTING Shopify listings so edited item data (extended properties, title, price, description) reaches the live channel. **Never creates** — a SKU not already live on `sub_source` → `unresolved` (use `list_to_shopify`). Per SKU: resolve → confirm SHOPIFY+`sub_source` mapping (channel-SKU table) → `OpenTemplatesByInventory` opens the item's **existing** GLT template (a safe read; returns top-level `Id`=TemplateId, `IsLocked`, `IsAllowedToRevise`, `NextSuggestedAction`, nested `Info.ActiveListingId`/`Status`) → `ProcessTemplates` with the revise action. **Action is data-driven:** `action=None` (auto) uses the template's `NextSuggestedAction` when allowed (typically "Update" for Shopify), else `"Revise"`; force with `action="Revise"/"Update"/…`. Locked / no-allowed-action templates → `unresolved`. **Read/selection path live-confirmed 18 Jun 2026; `ProcessTemplates` revise push spec-based, not yet live-run** (changes a real customer-facing listing — start with one SKU). `sub_source`→`ChannelId` from the configurator catalogue. |

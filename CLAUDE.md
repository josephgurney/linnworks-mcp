...
**Current version: 1.24.0** — 70 tools. See `pyproject.toml` for full metadata.
...
| `delete_categories(category_ids, force=False, confirmed_count, dry_run=True)` | `Inventory/DeleteCategoryById` (+ sweep unless `force`) | 10 | **IRREVERSIBLE.** Deletes specific ids. **Refuses non-empty categories** (marks them `blocked`) unless `force=True` — which skips the sweep and deletes regardless (their items → Default). Default id always blocked; unknown ids → `error`. Per-id manifest (name, item_count, status). |
| `delete_empty_categories(confirmed_count, dry_run=True)` | `Stock/GetStockItems` (sweep) + `Inventory/DeleteCategoryById` | 10 | **IRREVERSIBLE — headline cleanup.** Sweeps the catalogue, finds every zero-item category (Default excluded), shows the manifest, and bulk-deletes on confirm. Sweep re-runs on the confirmed call to re-verify emptiness. Live sweep found **62 of 181 categories empty** (junk like `#REF!`/`AUCTION`/`SHOWS` + archived ranges). |
...
| `Inventory/DeleteCategoryById` | POST | `{"categoryId": "guid"}` **unwrapped** | Deletes a category by id; empty 2xx body on success (subsequent GetCategories no longer lists it). A **non-empty category's items are reassigned to Default** (items are NOT deleted). Never delete the zero-GUID Default category. Wrapped by `delete_categories` + `delete_empty_categories`. **Live-tested 7 Jul 2026** (v1.24.0) |
| `Inventory/GetInventoryItemsCount` | GET | `?includeDeleted=<bool>&includeArchived=<bool>` | Returns a single int — the **global** item count (32,125 in this tenant). Only takes deleted/archived flags — **no category filter**, so it can't count items per category. Used only as a sanity check; emptiness is derived by the full `Stock/GetStockItems` sweep instead. **Live-confirmed 7 Jul 2026** |
